...
public class AddressBook extends JFrame {
    ...
    jbtUpdate.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				long j = 0;
				for(long i = 0; i <= fileSize; i = RECORD_SIZE*(2*j++)){
					if ((jtfName.getText()).equals(readAddressCheck(i))){
						//long currentPosition = raf.getFilePointer();
						raf.seek(i);
						writeAddress();
						 //j++;
  ...
  public String readAddressCheck(long position) throws IOException{
	  raf.seek(position);
	  String name;
	    return name = FixedLengthStringIO.readFixedLengthString(
	      NAME_SIZE, raf);
	    /*String street = FixedLengthStringIO.readFixedLengthString(
	      STREET_SIZE, raf);
	    String city = FixedLengthStringIO.readFixedLengthString(
